...
const verify = async (name, contextOptions) => {
  ...
  try {
    ...
    const contributorMenu = page.getByRole('dialog', { name: 'Menú principal' });
    if (await contributorMenu.getByRole('button', { name: 'Acceso voluntarios' }).count()) throw new Error(`${name}: contributor menu must not expose volunteer access`);
...
const verifyDirectLogin = async (name, contextOptions) => {
  ...
  try {
    ...
    console.log(`[ok] ${name}: menu contribution login preserves the create-resource intent.`);
  } finally { await browser.close(); }
};
